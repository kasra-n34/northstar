  ...
  return (
    <div style={{ width: 196, flexShrink: 0, background: "var(--bg1)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column", height: "100vh" }}>
        ...
        {NAV.map(item => {
          ...
          return (
            <button key={item.id} className="nav-btn" onClick={() => onNav(item.id)} style={{ display: "flex", alignItems: "center", gap: 9, padding: "8px 11px", width: "100%", textAlign: "left", background: isActive ? "var(--bg3)" : "transparent", border: isActive ? "1px solid var(--border2)" : "1px solid transparent", color: isActive ? "var(--text)" : "var(--text2)", transition: "all 0.15s" }}>
              <span style={{ fontSize: 12, color: isActive ? (pillar?.color || "var(--y)") : "var(--text3)", width: 14, textAlign: "center" }}>{item.icon}</span>
              <Mono s={{ fontSize: 9, letterSpacing: 1, flex: 1 }}>{item.label}</Mono>
              {item.id === "missions" && pending > 0 && <span style={{ background: "var(--o)", color: "#000", fontFamily: "'DM Mono',monospace", fontSize: 8, padding: "1px 5px", borderRadius: 999, fontWeight: 600 }}>{pending}</span>}
              {item.id === "missions" && activeMissions > 0 && pending === 0 && <span style={{ background: "var(--c)", color: "#000", fontFamily: "'DM Mono',monospace", fontSize: 8, padding: "1px 5px", borderRadius: 999, fontWeight: 600 }}>{activeMissions}</span>}
      ...
      <div style={{ padding: "14px 18px", borderTop: "1px solid var(--border)" }}>
        <Mono s={{ fontSize: 8, color: "var(--text3)", letterSpacing: 1.5, display: "block", marginBottom: 8 }}>PILLARS ACTIVE</Mono>
