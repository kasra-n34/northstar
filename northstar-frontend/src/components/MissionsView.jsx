  ...
  return (
    <div style={{ marginTop: 10 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
        <Mono s={{ fontSize: 8, color: preview ? "var(--text3)" : (allDone ? "var(--g)" : pillarColor), letterSpacing: 1.5 }}>
          {preview ? "SUBTASKS PREVIEW" : "SUBTASKS"}
        </Mono>
        <Mono s={{ fontSize: 8, color: preview ? "var(--text3)" : (allDone ? "var(--g)" : "var(--text2)") }}>
        ...
        {subtasks.map((st, i) => {
          ...
          const box = (
            <div style={{
              ...
              {isDone && <span style={{ color: pillarColor, fontSize: 8, lineHeight: 1 }}>✓</span>}
            </div>
          );
          if (preview) {
            return (
              <div key={st.id || i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "3px 0", opacity: 0.6 }}>
                {box}
                <span style={{ fontSize: 11, color: "var(--text3)", lineHeight: 1.4 }}>{st.label || st}</span>
              </div>
            );
          }
          return (
            ...
            >
              {box}
              <span style={{ fontSize: 12, color: isDone ? "var(--text3)" : "var(--text2)", textDecoration: isDone ? "line-through" : "none", lineHeight: 1.4 }}>{st.label}</span>
            </button>
          );
        })}
      </div>
...
function CounterControl({ value, max, missionId, onIncrement, color = "var(--c)" }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 10 }}>
      ...
      >−</button>
      <Mono s={{ fontSize: 12, color, minWidth: 48, textAlign: "center" }}>{value}/{max}</Mono>
      <button
        onClick={() => onIncrement(missionId, 1)}
        disabled={value >= max}
...
function PendingMissionCard({ m, onAccept, onAcceptRecurring, onDecline }) {
  ...
  const inputStyle = { width: "100%", padding: "7px 9px", fontSize: 12, background: "var(--bg3)", border: "1px solid var(--border2)", color: "var(--text)", fontFamily: "inherit", lineHeight: 1.5 };
  const labelStyle = { display: "block", fontFamily: "'DM Mono',monospace", fontSize: 8, color: "var(--text3)", letterSpacing: 1.5, marginBottom: 5 };
      ...
      <div style={{ paddingLeft: 10 }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          ...
          <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
            {!editing && <button onClick={() => { setEditing(true); setExp(false); }} style={{ background: "none", border: "1px solid var(--border)", color: "var(--text3)", padding: "2px 9px", fontFamily: "'DM Mono',monospace", fontSize: 8, letterSpacing: 1 }}>EDIT</button>}
            {!editing && <button onClick={() => setExp(e => !e)} style={{ background: "none", border: "none", color: "var(--text3)", fontSize: 11, padding: "0 3px" }}>{exp ? "▲" : "▼"}</button>}
          </div>
        </div>
            ...
            return (
              <div style={{ marginTop: 10 }}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                  <Mono s={{ fontSize: 8, color: "var(--text3)", letterSpacing: 1.5 }}>
                    {effectiveType === "recurring" ? "WEEKLY TARGET" : "PROGRESS TARGET"}
                  </Mono>
                  <Mono s={{ fontSize: 8, color: "var(--text3)" }}>0/{tc}</Mono>
                </div>
                ...
                <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  {tc > 12 && <Mono s={{ fontSize: 9, color: "var(--text3)" }}>+{tc - 12} more</Mono>}
                </div>
              </div>
            );
          }
        ...
        {exp && !editing && (
          <div style={{ marginTop: 10, marginBottom: 8 }}>
            <div style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.6, marginBottom: 6 }}>{saved.description}</div>
            <div style={{ fontSize: 11, color: "var(--text3)", fontStyle: "italic" }}>Why: {saved.why}</div>
          </div>
        )}
        ...
        {editing && (
          <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 11 }}>
            ...
            <div style={{ display: "flex", gap: 8, paddingTop: 4 }}>
              <button onClick={handleSaveEdits} style={{ flex: 1, background: (p?.color || "var(--c)") + "22", border: `1px solid ${(p?.color || "var(--c)")}55`, color: p?.color || "var(--c)", padding: "7px 0", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1 }}>
                ✓ SAVE CHANGES
              </button>
              <button onClick={handleDiscardEdits} style={{ background: "none", border: "1px solid var(--border)", color: "var(--text3)", padding: "7px 14px", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1 }}>
                ✕ DISCARD
              </button>
            </div>
          </div>
        ...
        {!editing && (
          <div style={{ display: "flex", gap: 8, marginTop: 12, paddingTop: 12, borderTop: "1px solid var(--border)" }}>
            <button onClick={handleAccept} style={{ flex: 1, background: "var(--c)22", border: "1px solid var(--c)55", color: "var(--c)", padding: "8px 0", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1 }}>
              {isSavedEdited ? "✓ ACCEPT EDITED" : effectiveType === "recurring" ? "✓ ADD TO RECURRING" : "✓ ACCEPT"}
            </button>
            <button onClick={() => onDecline(m.id)} style={{ flex: 1, background: "var(--r)11", border: "1px solid var(--r)33", color: "var(--r)", padding: "8px 0", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1 }}>✕ DECLINE</button>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="card" style={{ background: done ? "var(--bg1)" : isOverdue ? "var(--r)08" : "var(--bg2)", border: `1px solid ${isOverdue ? "var(--r)44" : "var(--border)"}`, padding: "12px 14px", marginBottom: 7, opacity: done ? 0.5 : 1, transition: "all 0.15s", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
        <button onClick={() => done ? onUncomplete(m.id) : onComplete(m.id)} style={{ width: 20, height: 20, border: `1.5px solid ${done ? p?.color : isOverdue ? "var(--r)" : "var(--border2)"}`, background: done ? (p?.color || "var(--c)") + "33" : "transparent", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", color: p?.color, fontSize: 11, marginTop: 2 }}>{done && "✓"}</button>
        <PillarDot id={m.pillar} size={6} />
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {exp && (
            <div style={{ marginTop: 8 }}>
              {m.description && <div style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.6, marginBottom: 5 }}>{m.description}</div>}
              {m.why && <div style={{ fontSize: 11, color: "var(--text3)", fontStyle: "italic" }}>Why: {m.why}</div>}
              {m.acceptedAt && <div style={{ fontSize: 10, color: "var(--text3)", marginTop: 4 }}>Accepted: {new Date(m.acceptedAt).toLocaleDateString("en-CA", { month: "short", day: "numeric" })} · Deadline: {m.deadlineDate ? new Date(m.deadlineDate).toLocaleDateString("en-CA", { month: "short", day: "numeric" }) : "—"}</div>}
            </div>
          )}
        </div>
        <div style={{ display: "flex", gap: 4, flexShrink: 0, alignItems: "center" }}>
          {!done && (confirmDelete
            ? <>
                <button onClick={() => onDelete(m.id)} style={{ background: "var(--r)22", border: "1px solid var(--r)55", color: "var(--r)", padding: "3px 8px", fontFamily: "'DM Mono',monospace", fontSize: 8, letterSpacing: 1 }}>CONFIRM</button>
                <button onClick={() => setConfirmDelete(false)} style={{ background: "none", border: "1px solid var(--border)", color: "var(--text3)", padding: "3px 8px", fontFamily: "'DM Mono',monospace", fontSize: 8, letterSpacing: 1 }}>CANCEL</button>
              </>
          ...
          <button onClick={() => setExp(e => !e)} style={{ background: "none", border: "none", color: "var(--text3)", fontSize: 11, padding: "0 3px" }}>{exp ? "▲" : "▼"}</button>
        </div>
      </div>
    </div>
  );
...
function RecurringCard({ m, onIncrementProgress, onDelete, onEdit }) {
  ...
  const inputStyle = { width: "100%", padding: "6px 8px", fontSize: 12, background: "var(--bg3)", border: "1px solid var(--border2)", color: "var(--text)", fontFamily: "inherit" };
  const labelStyle = { display: "block", fontFamily: "'DM Mono',monospace", fontSize: 8, color: "var(--text3)", letterSpacing: 1.5, marginBottom: 4 };
    ...
    <div style={{ background: done ? "var(--g)08" : "var(--bg2)", border: `1px solid ${editing ? (p?.color || "var(--border)") + "66" : done ? "var(--g)44" : "var(--border)"}`, padding: "12px 14px", marginBottom: 7, position: "relative", overflow: "hidden", transition: "all 0.15s" }}>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
        <div style={{ width: 20, height: 20, border: `1.5px solid ${color}`, background: done ? color + "33" : "transparent", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", marginTop: 2 }}>
          {done && <span style={{ color, fontSize: 9 }}>✓</span>}
        </div>
          ...
          {editing ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <div style={{ display: "flex", gap: 7, paddingTop: 2 }}>
                <button onClick={handleSave} style={{ flex: 1, background: (p?.color || "var(--p)") + "22", border: `1px solid ${(p?.color || "var(--p)")}55`, color: p?.color || "var(--p)", padding: "7px 0", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1 }}>✓ SAVE</button>
                <button onClick={() => { setDraft({ title: m.title, targetCount: m.targetCount || 1, difficulty: m.difficulty || "Medium", pillar: m.pillar }); setEditing(false); }} style={{ background: "none", border: "1px solid var(--border)", color: "var(--text3)", padding: "7px 14px", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1 }}>✕ CANCEL</button>
              </div>
            </div>
          ) : (
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                    <Mono s={{ fontSize: 8, color: done ? "var(--g)" : "var(--text3)", letterSpacing: 1.5 }}>THIS WEEK</Mono>
                    <Mono s={{ fontSize: 8, color }}>{progress}/{target}</Mono>
                  </div>
              ...
              {exp && (
                <div style={{ marginTop: 10 }}>
                  {m.description && <div style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.6, marginBottom: 5 }}>{m.description}</div>}
                  {m.why && <div style={{ fontSize: 11, color: "var(--text3)", fontStyle: "italic" }}>Why: {m.why}</div>}
                  <Mono s={{ fontSize: 8, color: "var(--text3)", display: "block", marginTop: 6 }}>Resets every Sunday · Added {new Date(m.acceptedAt).toLocaleDateString("en-CA", { month: "short", day: "numeric" })}</Mono>
                </div>
              )}
            </>
          )}
        ...
        {!editing && (
          <div style={{ display: "flex", gap: 4, flexShrink: 0, alignItems: "center" }}>
            <button onClick={() => setEditing(true)} style={{ background: "none", border: "1px solid var(--border)", color: "var(--text3)", padding: "2px 8px", fontFamily: "'DM Mono',monospace", fontSize: 8, letterSpacing: 1 }}>EDIT</button>
            {confirmDelete
              ? <>
                  <button onClick={() => onDelete(m.id)} style={{ background: "var(--r)22", border: "1px solid var(--r)55", color: "var(--r)", padding: "3px 8px", fontFamily: "'DM Mono',monospace", fontSize: 8, letterSpacing: 1 }}>CONFIRM</button>
                  <button onClick={() => setConfirmDelete(false)} style={{ background: "none", border: "1px solid var(--border)", color: "var(--text3)", padding: "3px 8px", fontFamily: "'DM Mono',monospace", fontSize: 8, letterSpacing: 1 }}>CANCEL</button>
...
function AddRecurringForm({ onAdd, onCancel }) {
  ...
  return (
    <div style={{ background: "var(--bg2)", border: `1px solid ${p?.color || "var(--border)"}66`, padding: 16, marginBottom: 14, position: "relative" }}>
      <div style={{ position: "absolute", top: 0, left: 0, bottom: 0, width: 2, background: p?.color || "var(--border)" }} />
      <div style={{ paddingLeft: 10 }}>
        <Mono s={{ fontSize: 9, color: p?.color || "var(--p)", letterSpacing: 2, display: "block", marginBottom: 12 }}>+ NEW RECURRING HABIT</Mono>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div>
            <label style={labelStyle}>HABIT NAME</label>
            <input
          ...
          <div style={{ display: "flex", gap: 8, paddingTop: 2 }}>
            <button
              onClick={handleAdd}
              disabled={!canSubmit}
              style={{ flex: 1, background: canSubmit ? (p?.color || "var(--p)") + "22" : "var(--bg3)", border: `1px solid ${canSubmit ? (p?.color || "var(--p)") + "55" : "var(--border)"}`, color: canSubmit ? p?.color || "var(--p)" : "var(--text3)", padding: "8px 0", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1, cursor: canSubmit ? "pointer" : "not-allowed" }}
            >+ ADD HABIT</button>
            <button onClick={onCancel} style={{ background: "none", border: "1px solid var(--border)", color: "var(--text3)", padding: "8px 14px", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1 }}>CANCEL</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      <div style={{ marginBottom: 22, paddingBottom: 18, borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          <div style={{ color: "var(--o)", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 3, marginBottom: 5 }}>◉ MISSIONS</div>
          <h2 style={{ fontFamily: "'Bebas Neue'", fontSize: 34, letterSpacing: 1, color: "var(--text)" }}>Mission Control</h2>
        </div>
        <Mono s={{ fontSize: 9, color: "var(--text3)" }}>New missions generated via ⟳ SYNC NORTHSTAR</Mono>
      </div>
        ...
        <>
          <div style={{ display: "flex", gap: 6, marginBottom: 18, flexWrap: "wrap", alignItems: "center" }}>
            <div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>
              {tabs.map(t => (
                <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ background: activeTab === t.id ? "var(--bg3)" : "none", border: `1px solid ${activeTab === t.id ? "var(--border2)" : "var(--border)"}`, color: activeTab === t.id ? "var(--text)" : "var(--text3)", padding: "6px 14px", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1 }}>
                  {t.label}
                </button>
              ))}
            </div>
            <div style={{ display: "flex", gap: 3, marginLeft: 8 }}>
              <button onClick={() => setPillarFilter("all")} style={{ background: pillarFilter === "all" ? "var(--bg3)" : "none", border: `1px solid ${pillarFilter === "all" ? "var(--border2)" : "var(--border)"}`, color: pillarFilter === "all" ? "var(--text)" : "var(--text3)", padding: "5px 11px", fontFamily: "'DM Mono',monospace", fontSize: 9 }}>ALL</button>
              {PILLARS.map(p => <button key={p.id} onClick={() => setPillarFilter(p.id)} style={{ background: pillarFilter === p.id ? p.color + "22" : "none", border: `1px solid ${pillarFilter === p.id ? p.color + "66" : "var(--border)"}`, color: pillarFilter === p.id ? p.color : "var(--text3)", padding: "5px 11px", fontFamily: "'DM Mono',monospace", fontSize: 9 }}>{p.icon}</button>)}
            </div>
          </div>
          ...
          {activeTab === "pending" && (
            applyFilter(pendingMissions).length === 0
              ? <div style={{ textAlign: "center", padding: "50px 0" }}><div style={{ fontFamily: "'Bebas Neue'", fontSize: 32, color: "var(--border2)", marginBottom: 10 }}>ALL CLEAR</div><div style={{ color: "var(--text3)", fontSize: 12 }}>No pending missions.</div></div>
              : <div>
                  <div style={{ background: "var(--o)0D", border: "1px solid var(--o)33", padding: "10px 14px", marginBottom: 16, fontSize: 12, color: "var(--text2)" }}>
          ...
          {activeTab === "active" && (
            applyFilter(activeMissions).length === 0
              ? <div style={{ textAlign: "center", padding: "50px 0" }}><div style={{ fontFamily: "'Bebas Neue'", fontSize: 32, color: "var(--border2)", marginBottom: 10 }}>NO ACTIVE MISSIONS</div><div style={{ color: "var(--text3)", fontSize: 12 }}>Accept missions from Pending to get started.</div></div>
              : <>
                  {grouped.overdue.length > 0 && (
                    <div style={{ marginBottom: 24 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                        <Mono s={{ fontSize: 9, color: "var(--r)", letterSpacing: 2 }}>⚠ OVERDUE</Mono>
                        <div style={{ flex: 1, height: 1, background: "var(--r)33" }} />
                        <Mono s={{ fontSize: 8, color: "var(--r)", letterSpacing: 1 }}>{grouped.overdue.length} MISSION{grouped.overdue.length !== 1 ? "S" : ""}</Mono>
                      </div>
                  ...
                  {[["today","TODAY"],["this_week","THIS WEEK"],["two_weeks","NEXT 2 WEEKS"],["this_month","THIS MONTH"]].map(([key,label]) => {
                    ...
                    return (
                      <div key={key} style={{ marginBottom: 24 }}>
                        <Mono s={{ fontSize: 9, color: "var(--text3)", letterSpacing: 2, display: "block", marginBottom: 10 }}>{label}</Mono>
          ...
          {activeTab === "recurring" && (
            applyFilter(recurringMissions).length === 0 && !showAddForm
              ? <div style={{ textAlign: "center", padding: "50px 0" }}>
                  ...
                  <div style={{ color: "var(--text3)", fontSize: 12, marginBottom: 20 }}>When Northstar generates a recurring mission, accept it here. Counts reset every Sunday.</div>
                  <button onClick={() => setShowAddForm(true)} style={{ background: "var(--p)22", border: "1px solid var(--p)55", color: "var(--p)", padding: "9px 20px", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1.5 }}>+ ADD HABIT MANUALLY</button>
                </div>
              : <div>
                  <div style={{ background: "var(--p)0D", border: "1px solid var(--p)33", padding: "10px 14px", marginBottom: 16 }}>
                    <div style={{ fontSize: 12, color: "var(--text2)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      {!showAddForm && <button onClick={() => setShowAddForm(true)} style={{ background: "var(--p)22", border: "1px solid var(--p)55", color: "var(--p)", padding: "5px 12px", fontFamily: "'DM Mono',monospace", fontSize: 8, letterSpacing: 1, flexShrink: 0, marginLeft: 12 }}>+ ADD HABIT</button>}
                    </div>
          ...
          {activeTab === "done" && (
            applyFilter(doneMissions).length === 0
              ...
              : <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                  <div style={{ fontSize: 11, color: "var(--muted)", textAlign: "right", paddingBottom: 4 }}>
