  ...
  return (
    <div style={{ maxWidth: 748 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: "var(--text3)", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1 }}>← BACK TO CONTACTS</button>
        <button onClick={onDelete} style={{ background: "none", border: "1px solid var(--r)33", color: "var(--r)", padding: "5px 10px", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1 }}>DELETE</button>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 22 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: "var(--text2)", marginTop: 2 }}>{contact.inferredRole} · {contact.inferredIndustry}</div>
        </div>
        {contact.networkValue && (
          <div style={{ marginLeft: "auto", textAlign: "center" }}>
            ...
            <Mono s={{ fontSize: 8, color: "var(--text3)" }}>NETWORK VALUE</Mono>
          </div>
        )}
      </div>
      ...
      {contact.potentialOpportunities?.length > 0 && (
        <div style={{ marginBottom: 18 }}>
          <Mono s={{ fontSize: 9, color: "var(--text3)", letterSpacing: 2, display: "block", marginBottom: 10 }}>OPPORTUNITIES</Mono>
          {contact.potentialOpportunities.map((o, i) => (
            <div key={i} style={{ display: "flex", gap: 10, marginBottom: 7 }}>
              <span style={{ color: "var(--p)", fontSize: 11, flexShrink: 0 }}>◈</span>
              <div style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.5 }}>{o}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {contact.howToApproach && (
        <div style={{ background: "var(--p)0D", border: "1px solid var(--p)33", padding: "14px 16px", marginBottom: 18 }}>
          <Mono s={{ fontSize: 8, color: "var(--p)", letterSpacing: 2, display: "block", marginBottom: 7 }}>HOW TO APPROACH</Mono>
      ...
      {contact.keyStrengths?.length > 0 && (
        <div style={{ marginBottom: 18 }}>
          <Mono s={{ fontSize: 9, color: "var(--text3)", letterSpacing: 2, display: "block", marginBottom: 8 }}>KEY STRENGTHS</Mono>
      ...
      {contact.followUpCadence && (
        <div style={{ marginBottom: 18 }}>
          <Mono s={{ fontSize: 8, color: "var(--text3)", letterSpacing: 1.5, display: "block", marginBottom: 5 }}>FOLLOW-UP CADENCE</Mono>
          <div style={{ fontSize: 12, color: "var(--text2)" }}>{contact.followUpCadence}</div>
        </div>
      )}

      {contact.clarifyingQuestions?.length > 0 && (
        <div style={{ marginBottom: 18 }}>
          <Mono s={{ fontSize: 9, color: "var(--o)", letterSpacing: 2, display: "block", marginBottom: 10 }}>NORTHSTAR WANTS TO KNOW MORE</Mono>
          {contact.clarifyingQuestions.map((q, i) => (
            <div key={i} style={{ marginBottom: 10 }}>
              <label style={{ display: "block", fontSize: 11, color: "var(--text2)", marginBottom: 5 }}>{q}</label>
              <input value={clarifyAnswers[q] || ""} onChange={e => setClarifyAnswers(a => ({ ...a, [q]: e.target.value }))} placeholder="Your answer..." style={{ width: "100%", padding: "8px 10px", fontSize: 11 }} />
            </div>
          ))}
          <button onClick={onResearch} disabled={loading} style={{ background: "var(--o)", color: "#000", border: "none", padding: "9px 20px", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1, display: "flex", alignItems: "center", gap: 7, marginTop: 8 }}>
      ...
      {contact.needsResearch && (
        <div style={{ background: "var(--o)11", border: "1px solid var(--o)33", padding: "12px 16px", marginBottom: 18, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ fontSize: 12, color: "var(--o)" }}>This contact hasn't been analyzed yet.</div>
          <button onClick={onResearch} disabled={loading} style={{ background: "var(--o)", color: "#000", border: "none", padding: "7px 16px", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1, display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <div>
        <Mono s={{ fontSize: 9, color: "var(--text3)", letterSpacing: 2, display: "block", marginBottom: 8 }}>NOTES</Mono>
        <textarea rows={3} value={notes} onChange={e => setNotes(e.target.value)} placeholder="Personal notes about this contact..." style={{ width: "100%", padding: "10px 12px", fontSize: 12, resize: "vertical", lineHeight: 1.6 }} />
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ marginBottom: 24, paddingBottom: 18, borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
        <div>
          <div style={{ color: "var(--p)", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 3, marginBottom: 5 }}>○ NETWORK</div>
          <h2 style={{ fontFamily: "'Bebas Neue'", fontSize: 34, letterSpacing: 1, color: "var(--text)" }}>Network Intelligence</h2>
        </div>
        <button onClick={generateSuggestions} disabled={suggestLoading || Object.keys(analyses).length === 0} style={{ background: "var(--p)", color: "#000", border: "none", padding: "8px 16px", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1, display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 22 }}>
        {tabs.map(t => <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ background: activeTab === t.id ? "var(--bg3)" : "none", border: `1px solid ${activeTab === t.id ? "var(--border2)" : "var(--border)"}`, color: activeTab === t.id ? "var(--text)" : "var(--text2)", padding: "6px 14px", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1 }}>{t.label}</button>)}
      </div>
      ...
      {activeTab === "contacts" && (
        selectedContact ? (
          ...
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            ...
            <button onClick={() => setActiveTab("add")} style={{ background: "var(--p)", color: "#000", border: "none", padding: "11px 24px", fontFamily: "'DM Mono',monospace", fontSize: 10, letterSpacing: 1.5 }}>ADD FIRST CONTACT →</button>
          </div>
        ) : (
              ...
              <button key={c.id} onClick={() => setSelectedContact(c)} className="card" style={{ background: "var(--bg1)", border: "1px solid var(--border)", padding: 16, textAlign: "left", transition: "all 0.2s", position: "relative" }}>
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: 10, marginBottom: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: "var(--text2)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.inferredRole || c.source}</div>
                  </div>
                </div>
        ...
        <div style={{ maxWidth: 704 }}>
          <div style={{ display: "flex", gap: 4, marginBottom: 20 }}>
            {[{ id: "url", label: "LINKEDIN / IG URL" }, { id: "paste", label: "PASTE BIO" }, { id: "csv", label: "IMPORT CSV" }].map(m => (
              <button key={m.id} onClick={() => setAddMode(m.id)} style={{ background: addMode === m.id ? "var(--p)22" : "none", border: `1px solid ${addMode === m.id ? "var(--p)66" : "var(--border)"}`, color: addMode === m.id ? "var(--p)" : "var(--text3)", padding: "6px 14px", fontFamily: "'DM Mono',monospace", fontSize: 9 }}>{m.label}</button>
            ))}
          </div>
          {addMode === "url" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.6 }}>Paste a public profile URL. Northstar will web-search publicly available info and analyse their network value.</div>
              <input value={urlInput} onChange={e => setUrlInput(e.target.value)} placeholder="https://instagram.com/username" style={{ width: "100%", padding: "11px 13px", fontSize: 12 }} />
              <button onClick={researchFromUrl} disabled={!urlInput.trim() || loading} style={{ alignSelf: "flex-start", background: urlInput.trim() ? "var(--p)" : "var(--bg2)", color: urlInput.trim() ? "#000" : "var(--text3)", border: "none", padding: "11px 22px", fontFamily: "'DM Mono',monospace", fontSize: 10, letterSpacing: 1.5, display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {addMode === "paste" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.6 }}>Paste someone's bio, About section, or any description.</div>
              <textarea rows={8} value={pasteInput} onChange={e => setPasteInput(e.target.value)} placeholder="Paste their bio or About section..." style={{ width: "100%", padding: "11px 13px", fontSize: 12, resize: "vertical", lineHeight: 1.6 }} />
              <button onClick={researchFromPaste} disabled={!pasteInput.trim() || loading} style={{ alignSelf: "flex-start", background: pasteInput.trim() ? "var(--p)" : "var(--bg2)", color: pasteInput.trim() ? "#000" : "var(--text3)", border: "none", padding: "11px 22px", fontFamily: "'DM Mono',monospace", fontSize: 10, letterSpacing: 1.5, display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {addMode === "csv" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.6 }}>Paste CSV rows: <Mono s={{ fontSize: 11, color: "var(--text3)" }}>Name, Role, Company, Notes</Mono></div>
              <textarea rows={6} value={csvInput} onChange={e => setCsvInput(e.target.value)} placeholder="John Smith, Founder, Fintech, Met at startup event" style={{ width: "100%", padding: "11px 13px", fontSize: 12, resize: "vertical", lineHeight: 1.6 }} />
              <button onClick={importCSV} disabled={!csvInput.trim()} style={{ alignSelf: "flex-start", background: csvInput.trim() ? "var(--p)" : "var(--bg2)", color: csvInput.trim() ? "#000" : "var(--text3)", border: "none", padding: "11px 22px", fontFamily: "'DM Mono',monospace", fontSize: 10, letterSpacing: 1.5 }}>IMPORT CONTACTS →</button>
            </div>
          )}
        </div>
      )}
              ...
              <div key={s.id} style={{ background: "var(--bg1)", border: `1px solid ${s.saved ? "var(--border)" : "var(--p)22"}`, padding: 18, opacity: s.saved ? 0.5 : 1 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 12 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "flex-end", flexShrink: 0 }}>
                    <div style={{ fontFamily: "'Bebas Neue'", fontSize: 24, color: "var(--p)", lineHeight: 1 }}>{s.priority}<span style={{ fontSize: 12, color: "var(--text3)" }}>/10</span></div>
                    {!s.saved && <button onClick={() => saveToContacts(s)} style={{ background: "var(--p)", color: "#000", border: "none", padding: "6px 12px", fontFamily: "'DM Mono',monospace", fontSize: 9, letterSpacing: 1 }}>SAVE →</button>}
                    {s.saved  && <Tag color="var(--g)">SAVED</Tag>}
                  </div>
                </div>
                <div style={{ background: "var(--bg2)", padding: "10px 14px", marginBottom: 8 }}>
                  <Mono s={{ fontSize: 8, color: "var(--p)", letterSpacing: 1.5, display: "block", marginBottom: 5 }}>OPPORTUNITY</Mono>
                  <div style={{ fontSize: 12, color: "var(--text2)", lineHeight: 1.5 }}>{s.opportunity}</div>
                </div>
                <div style={{ fontSize: 11, color: "var(--text3)", lineHeight: 1.5 }}><span style={{ color: "var(--text2)" }}>Find them:</span> {s.findWhere}</div>
                {s.approach && <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 5 }}><span style={{ color: "var(--text2)" }}>Approach:</span> {s.approach}</div>}
              </div>
            ))}
          </div>
        )
